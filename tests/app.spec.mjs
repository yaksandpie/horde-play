...
test("the decks screen lists the built-in hordes", async ({ app }) => {
  ...
  // However many ship: a count typed in here goes stale with every new deck.
  await expect(rows).toHaveCount(await app.evaluate(() => window.__horde.BUNDLED_DECKS.length));
