...
- **Static checks** — builds the site, which is what catches an unresolved
  ...
  service worker reference is really sitting there in `_site`.
- **Service worker cache version** — builds a copy of the source, perturbs
  the app shell, builds again, and checks the version moved. Nobody bumps it
  by hand any more, so what's worth checking is that the build's own hashing
  still works.
- **Browser tests** — Playwright drives real Chromium against the real page:
  ...
  and an import. `tests/a11y.spec.mjs` runs axe-core against WCAG 2.1 A and AA
